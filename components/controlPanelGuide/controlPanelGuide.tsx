...
export default function ControlPanelGuide({
  ...
  intoTest = { id: "", title: "", description: "" },
}:
  | {
      description?: string;
      howToPlay?: string;
      intoTest?: { id: string; title: string; description: string };
    }
  | any) {
  ...
  const title = intoTest.title
    ? `${intoTest.title} ${intoTest.id}`
  ...
  return (
    <div className="w-full h-full px-1 lg:px-7 flex flex-col justify-center space-y-2">
      ...
      <div>{intoTest.description ? intoTest.description : description}</div>
