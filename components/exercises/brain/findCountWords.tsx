...
export default function FindCountWords({
  ...
  const generateItems = useCallback(() => {
    ...
    // guarantee at least one instance of the frameTarget appears
    if (!arr.some((a) => a.text === frameTarget)) {
      if (arr.length > 0) {
        const idx = Math.floor(Math.random() * arr.length);
        arr[idx].text = frameTarget;
      } else {
        // fallback: push the target
        arr.push({ text: frameTarget, x: 50, y: 50 });
      }
    }
