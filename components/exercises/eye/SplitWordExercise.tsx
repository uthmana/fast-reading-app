...
export default function SplitWordMirrorExercise({ controls, onFinishTest, wordList = [] }: Props) {
    const distance = (controls?.distance ?? 3) * 20; // MUCH SMALLER, perfect now
    ...
    const intervalMs = useMemo(() => {
        let ms =
            typeof rawSpeed === "number" ? rawSpeed : parseInt(String(rawSpeed || 1200), 10);
        if (isNaN(ms)) ms = 1200;
        if (ms <= 50) ms *= 1000;
